...
namespace http{
	...
	_ERROR_CODE http_server::setHtml(std::string html){
		if (html.length() <= 0)
			this->_response_message._message_body = "<html><body>"
			"<form name = 'restorefile' method = 'POST' action = 'restore_config.cgi' ENCTYPE = 'multipart/form-data'>"
		"<input type = 'FILE' name = 'restoreFilename' value = '' size = '40' maxlength = '128'>"
		"<input type=\"hidden\" name=\"message\" value=\"\">"
			"<INPUT TYPE = 'submit' name = 'restorefile' txtid = 'restore'></form></body></html>";
		else
	...
	//function: recv aay bytes from socket
	...
	_ERROR_CODE http_server::init_http_request(SOCKET& socket)
	{
		...
		while (tmp != std::string("\r\n"))
		{
			...
			else if (std::string::npos != tmp.find("Content-Length"))
			{
				...
				_request_message._request_header._Content_Length = _request_message._request_header._Content_Length == "" ? std::string("0"):_request_message._request_header._Content_Length;
			}
			else
				;
			tmp = recvLine(socket);
		...
		if (std::string::npos != _request_message._request_header._Content_Type.find("multipart/form-data")){
			...
			if (std::string::npos != bound){
				...
				std::string tempstr = recvLine(socket);//pass boundary line
				std::string disposition = recvLine(socket);
				std::string type = recvLine(socket);
				tempstr += disposition + type;
				//std::cout << disposition << type;
				int pos = 0;
				std::string filename = "";
				if (std::string::npos != (pos = disposition.rfind("\\")))
					...
					filename = disposition.substr(pos + strlen("filename=\""), disposition.length() - pos - strlen("filename=\"") - 3);

				tempstr += recvLine(socket);//pass empty line
				char data = '\0';
				int iread = 0;
				std::string fr="";
				bool found = false;
				...
				ofs.close();
				int sxia = stol(_request_message._request_header._Content_Length) - tempstr.length() - iread;
				
				for (int i = 0; i<sxia;i++)
					tempstr += this->recvByte(socket);
				std::cout << tempstr;
				std::cout << tempstr.length() + iread;
			}
		}
